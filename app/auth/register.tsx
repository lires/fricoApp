...
// ⬇️ expo-checkbox -> import par défaut
...
export default function Register() {
  ...
  const handleRegister = async () => {
    ...
    if (!isFormValid) {
      setErrorMessage("Veuillez compléter correctement le formulaire.");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.replace("/"); // ✅ sur web on pointe vers "/"
      } else {
        // le backend renvoie { content: "..." } en cas d'erreur
        setErrorMessage(data?.content || data?.message || "Une erreur est survenue.");
      }
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.passwordRequirementContainer}>
        <Text style={[styles.passwordRequirementText, { color: hasUppercase ? "green" : "red" }]}>
          Contient une majuscule
        </Text>
        <Text style={[styles.passwordRequirementText, { color: hasNumber ? "green" : "red" }]}>
          Contient un chiffre
        </Text>
        <Text style={[styles.passwordRequirementText, { color: hasSpecialChar ? "green" : "red" }]}>
          Contient un caractère spécial
        </Text>
        <Text style={[styles.passwordRequirementText, { color: password.length >= 7 ? "green" : "red" }]}>
      ...
      <View style={styles.checkboxContainer}>
        ...
        <Text style={styles.checkboxText}>
          ...
          <Text style={styles.link} onPress={() => router.push("/tos-privacy/privacy")}>
            politique de confidentialité
          </Text>{" "}
          et aux{" "}
          <Text style={styles.link} onPress={() => router.push("/tos-privacy/terms")}>
      ...
      <TouchableOpacity
        ...
        style={[styles.button, (!isFormValid || isLoading) && { opacity: 0.7 }]}
